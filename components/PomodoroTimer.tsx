...
interface PomodoroTimerProps {
  ...
  onSessionComplete: (questions?: number, correct?: number, duration?: number, sessionType?: 'teoria' | 'questao') => void;
  animationKey?: number;
}
...
const PomodoroTimer: React.FC<PomodoroTimerProps> = ({
  ...
  const [sessionType, setSessionType] = useState<'teoria' | 'questao'>('questao');
  ...
  const handleQuestionsSubmit = () => {
    ...
    onSessionComplete(tempQuestionsResolved, tempCorrectQuestions, elapsedMinutes, sessionType);
  ...
  const handleSkipQuestions = () => {
    ...
    onSessionComplete(0, 0, elapsedMinutes, sessionType);
  };
    ...
    <>
      <div className="pomodoro-container" ref={containerRef}>
        <div className="timer-floating">
          ...
          <div className="session-type-switch">
            <button
              className={`session-type-btn ${sessionType === 'teoria' ? 'active' : ''}`}
              onClick={() => { soundService.playClick(); setSessionType('teoria'); }}
              disabled={isRunning}
            >
              Teoria
            </button>
            <button
              className={`session-type-btn ${sessionType === 'questao' ? 'active' : ''}`}
              onClick={() => { soundService.playClick(); setSessionType('questao'); }}
              disabled={isRunning}
            >
              Questão
            </button>
